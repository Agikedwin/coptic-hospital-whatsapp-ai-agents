...
@router.get("", response_model=schemas.Page[schemas.ClientRead])
def list_items(
...
@router.get("/{item_id}", response_model=schemas.ClientRead)
...
def create_item(payload: schemas.ClientCreate, db: Session = Depends(get_db)):
    try:
...
@router.patch("/{item_id}", response_model=schemas.ClientRead)
def update_item(item_id: int, payload: schemas.ClientUpdate, db: Session = Depends(get_db)):
    obj = crud.get(db, item_id)
    if obj is None:
        raise HTTPException(status_code=404, detail="Client not found")
    try:
